...
void neutrinolightbulb(Mesh* pm, const Real bdt){
  ...
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
    ...
    if (r>0.0){
      ...
      u0(m,IEN,k,j,i) += alpha*vol*bdt*w0(m,IDN,k,j,i)*ut *((0.0079*Q*(pow((Tnu/4.0),2)/pow(r,2))) - factor*BB*pow(p,1.5))*z;
      u0(m,IM1,k,j,i) += alpha*vol*bdt*w0(m,IDN,k,j,i)*u_x*((0.0079*Q*(pow((Tnu/4.0),2)/pow(r,2))) - factor*BB*pow(p,1.5))*z;
      u0(m,IM2,k,j,i) += alpha*vol*bdt*w0(m,IDN,k,j,i)*u_y*((0.0079*Q*(pow((Tnu/4.0),2)/pow(r,2))) - factor*BB*pow(p,1.5))*z;
      u0(m,IM3,k,j,i) += alpha*vol*bdt*w0(m,IDN,k,j,i)*u_z*((0.0079*Q*(pow((Tnu/4.0),2)/pow(r,2))) - factor*BB*pow(p,1.5))*z;
      u0(m,nvars,k,j,i) += alpha*vol*bdt*(w0(m,IDN,k,j,i)*vol*ut)*(lambda1 - factor2*lambda2*w0(m,nvars,k,j,i))*z;
