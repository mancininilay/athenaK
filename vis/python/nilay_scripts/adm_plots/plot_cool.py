...
def main(**kwargs):
    ...
    derived_dependencies['cool'] = ('eint','dens', 'bcc1', 'bcc2', 'bcc3','velx', 'vely', 'velz','s_00')
    ...
    derived_dependencies['cons_mhd_rel_z'] = ('dens', 'eint', 'velx', 'vely', 'velz',
                                              'bcc1', 'bcc2', 'bcc3')
    derived_dependencies['adm'] = ('adm_gxx', 'adm_gxy', 'adm_gxz', 'adm_gyy', 'adm_gyz', 'adm_gzz', 'adm_alpha', 'adm_betax', 'adm_betay', 'adm_betaz')   
    ...
    with open(kwargs['data_file2'], 'rb') as f:
        ...
        variable_format = 'f' if variable_size == 4 else 'd'
        num_variables_base = len(variable_names_base)
        if kwargs['variable2'][:8] == 'derived:':
            variable_name = kwargs['variable2'][8:]
            if variable_name not in derived_dependencies:
                raise RuntimeError('Derived variable "{0}" not valid; options are " \
                        "{{{1}}}.'.format(variable_name,
                                          ', '.join(derived_dependencies.keys())))
            ...
            for dependency in derived_dependencies[variable_name]:
                ...
                while variable_names_base[variable_ind] != dependency:
                    variable_ind += 1
                variable_inds.append(variable_ind)
        elif kwargs['variable2'] == 'level':
            variable_name = kwargs['variable2']
            variable_names = [variable_name]
            variable_inds = [-1]
        else:
            variable_name = kwargs['variable2']
        ...
        while f.tell() < file_size:
            ...
            for ind, name in zip(variable_inds_sorted, variable_names_sorted):
                if ind == -1:
                    if kwargs['dimension'] == 'x':
                        quantities2[name].append(np.full((block_nz, block_ny),
                                                        block_level))
                    if kwargs['dimension'] == 'y':
                        quantities2[name].append(np.full((block_nz, block_nx),
                                                        block_level))
                    if kwargs['dimension'] == 'z':
                        quantities2[name].append(np.full((block_ny, block_nx),
                                                        block_level))
                else:
                    f.seek(cell_data_start + ind * variable_data_size, 0)
                    cell_data = (np.array(struct.unpack(block_cell_format,
                    ...
                    block_ind = block_ind_for_level[block_level]
                    if kwargs['dimension'] == 'x':
                        quantities2[name].append(cell_data[:, :, block_ind])
                    if kwargs['dimension'] == 'y':
                        quantities2[name].append(cell_data[:, block_ind, :])
                    if kwargs['dimension'] == 'z':
                        quantities2[name].append(cell_data[block_ind, :, :])
    ...
    # Prepare to calculate derived quantity
    for name in variable_names_sorted:
        quantities2[name] = np.array(quantities2[name])

    gxx = quantities2['adm_gxx']
    gxy = quantities2['adm_gxy']
    gxz = quantities2['adm_gxz']
    gyy = quantities2['adm_gyy']
    gyz = quantities2['adm_gyz']
    gzz = quantities2['adm_gzz']
    alpha = quantities2['adm_alpha']
    betax = quantities2['adm_betax']
    betay = quantities2['adm_betay']
    betaz = quantities2['adm_betaz']

    utildex = quantities['velx']
    utildey = quantities['vely']
    utildez = quantities['velz']
    Bx = quantities['bcc1']
    By = quantities['bcc2']
    Bz = quantities['bcc3']
    rho = quantities['dens']
    ...
    Bv = gxx*Bx*utildex + gxy*Bx*utildey + gxz*Bx*utildez + gxy*By*utildex + gyy*By*utildey + gyz*By*utildez + gxz*Bz*utildex + gyz*Bz*utildey + gzz*Bz*utildez
    ...
    if kwargs['variable'] in \
        ...
        if kwargs['variable'] == 'derived:pgas':
            quantity = pgas
        elif kwargs['variable'] == 'derived:beta':
            quantity = quantities['eint'] / (0.5 * bsq)
        elif kwargs['variable'] == 'derived:sigma':
            quantity = bsq / quantities['dens']
        elif kwargs['variable'] == 'derived:vr':
            quantity = vr
        elif kwargs['variable'] == 'derived:vphi':
            quantity = vphi
        elif kwargs['variable'] == 'derived:Br':
            quantity = Br
    ...
    vmin_rhocgs, vmax_rhocgs = np.nanmin(rhocgs), np.nanmax(rhocgs)
    vmin_T, vmax_T = np.nanmin(T), np.nanmax(T)
    vmin_ye, vmax_ye = np.nanmin(ye), np.nanmax(ye)
    vmin_s, vmax_s = np.nanmin(s), np.nanmax(s)

    # Choose colormap norms
    norm_rhocgs = colors.LogNorm(vmin_rhocgs, vmax_rhocgs)
    norm_T = colors.LogNorm(vmin_T, vmax_T)
    norm_ye = colors.Normalize(vmin_ye, vmax_ye)
    norm_s = colors.Normalize(vmin_s, vmax_s)

    # Prepare figure
    fig, ax = plt.subplots(2, 2, figsize=(10, 10))

    # Plot data based on x and y values
    for block_num in range(num_blocks_used):
        if x[block_num] < 0 and y[block_num] > 0:
            img = ax[1, 0].imshow(rhocgs[block_num], cmap='plasma', norm=norm_rhocgs, interpolation='none', origin='lower', extent=extents[block_num])
            cbar = fig.colorbar(img, ax=ax[1, 0], orientation='vertical', fraction=0.046, pad=0.04)
            cbar.set_label(r'$\rho$ [g/cm$^3$]')

        elif x[block_num] > 0 and y[block_num] > 0:
            img = ax[1, 1].imshow(T[block_num], cmap='inferno', norm=norm_T, interpolation='none', origin='lower', extent=extents[block_num])
            cbar = fig.colorbar(img, ax=ax[1, 1], orientation='vertical', fraction=0.046, pad=0.04)
            cbar.set_label(r'$T$ [MeV]')

        elif x[block_num] < 0 and y[block_num] < 0:
            img = ax[0, 0].imshow(ye[block_num], cmap='viridis', norm=norm_ye, interpolation='none', origin='lower', extent=extents[block_num])
            cbar = fig.colorbar(img, ax=ax[0, 0], orientation='vertical', fraction=0.046, pad=0.04)
            cbar.set_label(r'$Y_e$')

        elif x[block_num] > 0 and y[block_num] < 0:
            img = ax[0, 1].imshow(s[block_num], cmap='pqlx', norm=norm_s, interpolation='none', origin='lower', extent=extents[block_num])
            cbar = fig.colorbar(img, ax=ax[0, 1], orientation='vertical', fraction=0.046, pad=0.04)
            cbar.set_label(r'$s$ [$k_B$/baryon]')
